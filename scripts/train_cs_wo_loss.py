...
class CrossAttEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        dim = hid_dim
        self.bilinear = nn.Bilinear(xtrain_dim, y_train_ohe_dim+train_pred_dim+1, bilinear_output_dim)
    ...
    def forward(self, inputs):
        ...
        bilinear_input = torch.cat((y_tr, torch.ones((y_tr.shape[0], 1)).to(device), train_pred), 1)
